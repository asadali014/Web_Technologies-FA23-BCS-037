...
const isLoggedIn = (req, res, next) => {
    ...
    if (req.xhr || (req.headers.accept && req.headers.accept.indexOf('json') > -1)) {
        return res.status(401).json({ success: false, error: 'Please log in to perform this action' });
    }
...
async function getSalesData() {
    const [totalOrders, revenueAgg, topProductAgg, recentOrders] = await Promise.all([
        ...
        // Top-selling product: unwind order items, sum quantities per product
        Order.aggregate([
            { $unwind: '$products' },
    ...
    const topProduct    = topProductAgg.length && topProductAgg[0].productInfo
        ? topProductAgg[0].productInfo.name
        : 'N/A';
...
app.get('/sales', isAdmin, async (req, res) => {
    try {
        ...
        res.render('sales', { salesData, title: 'Sales Dashboard — Daraz', activePage: 'sales' });
    } catch (err) {
        console.error('Sales route error:', err);
        res.status(500).send('<h2>Server error loading sales data</h2>');
    }
});

// ── Sales Data JSON Endpoint (for AJAX polling) ─────────────────
app.get('/api/sales-data', isAdmin, async (req, res) => {
    try {
        ...
        res.json({
            ...
            topProduct:    salesData.topProduct,
            recentOrders:  salesData.recentOrders
        });
    } catch (err) {
        console.error('API sales-data error:', err);
...
app.post('/register', async (req, res) => {
    ...
    if (errors.length > 0) {
        return res.render('auth/register', { layout: false, errors, name, email, error_msg: errors.map(e=>e.msg).join(', ') });
    }
...
app.get('/products', async (req, res) => {
    try {
        ...
        res.render('products', { layout: false, products, totalProducts, totalPages, currentPage: safePage, categories, filters, buildQuery });
    } catch (err) {
        console.error(err);
...
app.get('/admin', isAdmin, async (req, res) => {
    try {
        ...
        const flash    = req.query.flash
            ...
            : null;
        res.render('admin/dashboard', { layout: false, products, stats, flash });
    } catch (err) {
        console.error(err);
...
app.get('/admin/products/new', isAdmin, (req, res) => {
    res.render('admin/add', { layout: false, flash: null, old: null });
});
...
app.post('/admin/products', isAdmin, upload.single('image'), async (req, res) => {
    try {
        ...
        console.error(err);
        res.render('admin/add', { layout: false, flash: { type: 'error', message: 'Failed to save: ' + err.message }, old: req.body });
    }
});
...
app.get('/admin/products/:id/edit', isAdmin, async (req, res) => {
    try {
        ...
        res.render('admin/edit', { layout: false, product, flash: null });
    } catch (err) {
